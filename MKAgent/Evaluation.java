...
public class Evaluation {
    public static int getValue(Board board, Side playerSide) {
        int value = getSeedDiff(board, mySide) + maxMoveScore(board, mySide, true);
    ...
    public static boolean canNextMove(Node node){
        if(node.getMove().getSide() == node.getWhosTurnNext())
            return true;
        return false;
    }

    public static boolean canCaptureScore(Node node){
        Node simulationnode = new Node(node);
        Board board = new Board(simulationnode.getBoard());
        Move move = new Move(simulationnode.getMove().getSide(), simulationnode.getMove().getHole());
        if (getMaxCaptureScore(board, simulationnode.getWhosTurnNext(), move) >= 1){
            return true;
        }
        return false;
    }
